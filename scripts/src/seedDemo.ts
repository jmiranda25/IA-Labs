...
async function clean() {
  ...
  const demoClerkUsers = clerkUserList.data.filter((u) =>
    u.emailAddresses.some((e) => e.emailAddress.endsWith(DOMAIN))
  );

  for (const u of demoClerkUsers) {
    for (const ea of u.emailAddresses) {
      if (ea.emailAddress === "mayckolco@gmail.com") {
        throw new Error("Guard triggered: mayckolco@gmail.com must not be deleted!");
      }
    }
  }

  const demoEmailPattern = "%" + DOMAIN;

  // Because demo users are inserted with users.id = Clerk user ID, the single
  // subquery "SELECT id FROM users WHERE email LIKE ..." matches the owner
  // column in every content table, whether it stores an FK reference or a plain
  // Clerk-ID string.  No secondary SELECT clerk_id subquery is needed.
